...
import { Clock, Calendar, CheckCircle, XCircle, AlertCircle, TrendingUp, BarChart3 } from 'lucide-react';
import { useAsistencia } from '../../hooks/useAsistencia';

const AttendanceList: React.FC = () => {
  const { asistenciaData, estadisticas, isLoading, error, refetch } = useAsistencia();
  ...
  const handleMonthChange = (mes: number, anio: number) => {
    ...
    // El hook ya maneja esto automáticamente
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          <div className="bg-white p-4 rounded-lg shadow border">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-2xl font-semibold text-gray-900">{asistenciaData.resumen.diasTrabajados}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg shadow border">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-2xl font-semibold text-gray-900">{asistenciaData.resumen.horasTotales}h</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg shadow border">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-2xl font-semibold text-gray-900">{asistenciaData.resumen.horasPromedio}h</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg shadow border">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <p className="text-sm font-medium text-gray-500">Ausentismos</p>
                <p className="text-2xl font-semibold text-gray-900">{asistenciaData.resumen.ausentismos}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg shadow border">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <p className="text-2xl font-semibold text-gray-900">{asistenciaData.resumen.llegadasTarde}</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="flex items-center">
                <div className="flex-1 bg-gray-200 rounded-full h-2 mr-3">
                  <div 
                    ...
                    style={{ width: `${Math.min(100, estadisticas.porcentajeCumplimiento)}%` }}
                  ></div>
                </div>
                <span className="text-sm font-medium">{estadisticas.porcentajeCumplimiento}%</span>
              </div>
              <p className="text-xs text-gray-500 mt-1">
                {estadisticas.horasReales}h / {estadisticas.horasObjetivo}h objetivo
              </p>
            </div>

            <div>
              <h4 className="text-sm font-medium text-gray-500 mb-2">Promedio de Ingreso</h4>
              <p className="text-2xl font-semibold text-gray-900">{estadisticas.promedioHoraIngreso}</p>
            </div>

            <div>
              <h4 className="text-sm font-medium text-gray-500 mb-2">Día Más Productivo</h4>
              {estadisticas.diasMasProductivos[0] && (
                <div>
                  <p className="text-lg font-medium text-gray-900">
                    {estadisticas.diasMasProductivos[0].horas}h
                  </p>
                  <p className="text-xs text-gray-500">
                    {new Date(estadisticas.diasMasProductivos[0].fecha).toLocaleDateString('es-CL')}
                  </p>
                </div>
              )}
            </div>
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {asistencia.observacion && (
                        <p className="text-xs text-gray-400 mt-1">{asistencia.observacion}</p>
                      )}
                    </div>
                    <div className="flex-shrink-0">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${getStatusColor(asistencia.estado)}`}>
                        {asistencia.estado}
                      </span>
                    </div>
                  </div>
